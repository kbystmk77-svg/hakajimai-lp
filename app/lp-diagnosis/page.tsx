...
import { DiagnosisBlock } from "@/components/sections/diagnosis-block"

function SimulatorButton({ label = "10秒で費用を見積る" }: { label?: string }) {
  return (
    <div className="relative z-10 flex flex-col items-center gap-1.5 py-6">
      <a
        href="#simulation"
        className="inline-flex items-center gap-2 rounded-full bg-primary px-7 py-3 text-sm font-semibold text-primary-foreground shadow-md transition-opacity hover:opacity-90"
      >
        {label}
      </a>
      <span className="text-xs text-muted-foreground">費用の目安を10秒で確認できます</span>
    </div>
  )
}

export const metadata: Metadata = {
  title: "墓じまいパートナーズ｜費用シミュレーター付き無料相談",
  description:
    "墓じまいの費用・進め方を3問でシミュレーション。お墓の形態・大きさ・遺骨の数を選ぶだけで目安がわかります。まずはお気軽にご相談ください。",
  robots: { index: false, follow: false },
}

/* ─── BottomShape（メインページと同じ実装） ─────────────── */
...
export default function LpDiagnosisPage() {
  return (
    <div className="min-h-screen bg-background">
      <Header simulatorHref="#simulation" />
      <main>
        ...
        <BottomShape fill={COLORS.white} shape="slant">
          <div className="bg-linen">
            <Media />
            <Empathy />
            <SimulatorButton label="まず費用の目安を確認する" />
          </div>
        </BottomShape>
        ...
        <div className="bg-slate-light">
          <SimulatorButton label="自分のケースで費用を見積る" />
        </div>
